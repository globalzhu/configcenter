...
<div id="propertyValuesApp">
    <el-row style="margin-bottom: 10px">
        <el-col :span="16">
            ...
            <el-select style="width: 400px" v-model="branchId" @change="findAllData" placeholder="请选择分支" size="medium" filterable clearable>
    ...
    <el-row style="margin-bottom: 10px" v-if="(appProperties[0] ? appProperties[0].app.appId : null) !== 'ALL_APP'">
        <el-col :span="2">
        ...
        <el-col :span="2">
            <span style="font-size: x-large;color: red;">{{ toShowingLock(appProperties[0] ? appProperties[0].app : null) }}</span>
        </el-col>
        ...
        <el-col :span="3" style="text-align: end;">
            <el-popover placement="top" v-model="lockVersionShowing" trigger="manual">
                <p>{{lockVersion ==='YES' ? '点击解封后，所有镜像将允许修改，确定解封吗？': '点击封版后，所有镜像将不能修改，确定封版吗？'}}</p>
                <div style="text-align: right; margin: 0">
                   <el-button type="text" size="mini" @click="lockVersionShowing = false">取消</el-button>
                   <el-button type="primary" size="mini" @click="lockBranchVersion">确定</el-button>
                </div>
            </el-popover>
            <el-button type="danger" icon="el-icon-star-off" size="small" @click="showLockVersionDialog">{{lockVersion==='YES' ? '已封版，点击解封' : '点击封版'}}</el-button>
        </el-col>
        <el-col :span="4" style="text-align: end">
            ...
            <el-button type="primary" icon="el-icon-upload" size="small" :disabled="!edited || showMode!=='table' || lockVersion==='YES'" @click="showReleaseBranchDialog">发布修改</el-button>
        </el-col>
    </el-row>
    <el-row style="margin-bottom: 10px" v-if="(appProperties[0] ? appProperties[0].app.appId : null) === 'ALL_APP'">
            <el-col :span="14" style="text-align: center;">
    ...
    <template v-if="showMode!=='all' && appId!=='ALL_APP' ">
            ...
            <template v-for="profileProperty in appProperty.profileProperties">
                <template v-if="showMode==='table' || appProperty.app.appId!==appId || profileProperty.profileId!==profileId">
                    <el-table v-if="profileProperty.properties.length > 0"
                        ...
                        <el-table-column prop="valueType" label="配置项类型" :resizable="false" width="115px">
                                                    <template slot-scope="{ row }">
                                                        <el-select v-if="row.editing" v-model="row.editingValueType" size="mini" placeholder="请选择配置项类型" style="width: 90%">
                                                            <el-option value="PRODUCT_FEATURE_VALUE" label="产品功能变量"></el-option>
                                                            <el-option value="TOB_ENV_VALUE" label="客户环境变量"></el-option>
                                                            <el-option value="TOB_INTEGRATE_VALUE" label="客户集成变量"></el-option>
                                                        </el-select>
                                                        <template v-else>
                                                            <template v-if="appProperty.app.appId === appId && profileProperty.profileId === profileId && differenceForMap.modifiedScopeKeys[row.key]">
                                                                <el-badge type="warning" value="改" class="badge-style">
                                                                    <el-tag v-if="row.valueType === 'PRODUCT_FEATURE_VALUE'" size="medium">产品功能变量</el-tag>
                                                                    <el-tag v-else-if="row.valueType === 'TOB_ENV_VALUE'" type="success" size="medium">客户环境变量</el-tag>
                                                                    <el-tag v-else-if="row.valueType === 'TOB_INTEGRATE_VALUE'" type="warning" size="medium">客户集成变量</el-tag>
                                                                </el-badge>
                                                            </template>
                                                            <div v-else>
                                                                <el-tag v-if="row.valueType === 'PRODUCT_FEATURE_VALUE'" size="medium">产品功能变量</el-tag>
                                ...
                                <template v-if="appProperty.app.appId === appId && profileProperty.profileId === profileId">
                                    <template v-if="differenceForMap.deletedKeys[row.key]">
                                        <el-tooltip content="恢复" placement="top" :open-delay="1000" :hide-after="3000">
                                            <el-button @click="addOrModifyPropertyValue(row.key, row.value, row.scope, row.valueType)" :disabled="manager.type === 'NORMAL' && row.propertyType !== 'READ_WRITE'" type="success" icon="el-icon-plus" size="mini" circle></el-button>
                                        </el-tooltip>
                                    </template>
                                    ...
                                    <el-row v-else>
                                        <el-col :span="12" style="text-align: center">
                                            <el-tooltip v-if="!row.editing" content="修改" placement="top" :open-delay="1000" :hide-after="3000">
                                                <el-button @click="startEditing(row)" type="primary" :disabled="(manager.type === 'NORMAL' && row.propertyType !== 'READ_WRITE') || lockVersion==='YES'" icon="el-icon-edit" size="mini" circle></el-button>
                                            </el-tooltip>
                                            <el-button-group v-else>
                                                <el-tooltip content="取消修改" placement="top" :open-delay="1000" :hide-after="3000">
                                                    <el-button @click="cancelEditing(row)" type="info" icon="el-icon-close" size="mini" circle></el-button>
                                        ...
                                        <el-col :span="12" style="text-align: center">
                                            <el-tooltip content="删除" placement="top" :open-delay="1000" :hide-after="3000">
                                                <el-button @click="deletePropertyValue(row.key)" type="danger" :disabled="(manager.type === 'NORMAL' && row.propertyType !== 'READ_WRITE') || lockVersion==='YES'" icon="el-icon-delete" size="mini" circle></el-button>
                                            </el-tooltip>
                                        </el-col>
                                    </el-row>
                                </template>
    ...
    <template v-else-if="showMode!=='all' && appId==='ALL_APP' ">
            <div v-for="appProperty in appProperties" style="margin-bottom: 30px;">
                <el-row v-if="appProperty.app.appId !== appId" style="margin-bottom: 10px">
                    <el-col :offset="4" :span="16" style="text-align: center;">
                        <span style="font-size: large;color: #67c23a;">{{ toShowingApp(appProperty.app) }}</span>
                ...
                <template v-for="profileProperty in appProperty.profileProperties">
                    <template v-if="showMode==='table' || appProperty.app.appId!==appId || profileProperty.profileId!==profileId">
                        <el-table v-if="profileProperty.properties.length > 0"
                            ...
                            <el-table-column prop="appId" label="应用名称" :resizable="false">
                                <template slot-scope="{ row }">
                                    <span class="propertyValue-text-style">{{ row.appId }}</span>
                                </template>
                            </el-table-column>
                            <el-table-column prop="key" label="配置key" :resizable="false">
                                <template slot-scope="{ row }">
                                    <span class="propertyValue-text-style">{{ row.key }}</span>
                                </template>
                            </el-table-column>
                            <el-table-column prop="value" label="配置value" :resizable="false">
                                <template slot-scope="{ row }">
                                    <span class="propertyValue-text-style">{{ row.value }}</span>
                                </template>
                            </el-table-column>
                            <el-table-column prop="memo" label="备注" :resizable="false">
                                <template slot-scope="{ row }">
    ...
    <el-dialog :visible.sync="addPropertyValueDialogShowing" :before-close="closeAddPropertyValueDialog" title="新增配置项" width="50%">
        <el-form ref="addPropertyValueForm" :model="addPropertyValueForm" label-width="20%">
            ...
            <el-form-item label="配置项类型" prop="valueType" :rules="[{required:true, message:'请选择配置项类型', trigger:'blur'}]">
                <el-select v-model="addPropertyValueForm.valueType" placeholder="请选择配置项类型" style="width: 90%">
                    <el-option value="PRODUCT_FEATURE_VALUE" label="产品功能变量"></el-option>
                    <el-option value="TOB_ENV_VALUE" label="客户环境变量"></el-option>
                    <el-option value="TOB_INTEGRATE_VALUE" label="客户集成变量"></el-option>
                </el-select>
...
const PropertyValues = {
    ...
    data: function () {
        return {
            ...
            branchId: window.localStorage.getItem(this.appId + '_' + this.profileId + '_cur_branch_id') || 'master',
            lockVersion: 'NO',
    ...
    methods: {
        findAllData: function () {
            window.localStorage.setItem(this.appId + '_' + this.profileId + '_cur_branch_id', this.branchId);
        ...
        submitEditingPropertyValuesInText: function () {
            ...
            let addOrModifies = function (index, callback) {
                if (index < addedOrModifiedPropertyValues.length) {
                    ...
                    theThis.doAddOrModifyPropertyValue(propertyValue.key, propertyValue.value, propertyValue.scope, propertyValue.valueType, function () {
        ...
        calcRowKey: function (row) {
            return row.appId + row.profileId + row.branchId + row.key;
        },
        ...
        saveEditing: function (property) {
            ...
            this.addOrModifyPropertyValue(property.key, property.editingValue, property.editingScope, property.editingValueType);
        },
        addOrModifyPropertyValue: function (key, value, scope, valueType, callback) {
            const theThis = this;
            this.doAddOrModifyPropertyValue(key, value, scope, valueType, function () {
                theThis.findPropertyValues(function () {
                    theThis.findDifference(callback);
                });
            });
        },
        doAddOrModifyPropertyValue: function (key, value, scope, valueType, callback) {
        ...
        lockBranchVersion: function () {
                    const theThis = this;
                    if (theThis.lockVersion === 'YES') {
                        this.doUnLockBranchVersion(this.appId, this.profileId, this.branchId, function (ret) {
                            theThis.lockVersion = 'NO'
                            theThis.lockVersionShowing = false;
                        });
                    } else {
                        this.doLockBranchVersion(this.appId, this.profileId, this.branchId, function (ret) {
                            theThis.lockVersion = 'YES'
                            theThis.lockVersionShowing = false;
                        });
                    }
                },
        ...
        addPropertyValue: function () {
            ...
            this.$refs.addPropertyValueForm.validate(function (valid) {
                ...
                theThis.addOrModifyPropertyValue(
                    ...
                    theThis.addPropertyValueForm.valueType,
                    function () {
        ...
        showLockVersionDialog: function () {
            this.lockVersionShowing = true;
        },
        ...
        toShowingLock: function (app) {
                    if (!app) {
                        return '';
                    }
                    let text = app.lockVersion;
                    if (text == 'YES') {
                        text = '[已封版]';
                    }
                    return text;
        ...
        doFindBranchRelease: function (appId, profileId, branchId, callback) {
            ...
            axios.get('../manage/branch/findBranch', {
                ...
                theThis.lockVersion = result.branch.lockVersion;
                callback(result.branch.release);
            });
        },
        doLockBranchVersion: function (appId, profileId, branchId, callback) {
                    axios.get('../manage/branch/lockBranchVersion', {
                        params: {
                            appId: appId,
                            profileId: profileId,
                            branchId: branchId
                        ...
                        Vue.prototype.$message.success('封版成功');
                        callback(result.success);
                    });
        },
        doUnLockBranchVersion: function (appId, profileId, branchId, callback) {
            axios.get('../manage/branch/unLockBranchVersion', {
                params: {
                    appId: appId,
                    profileId: profileId,
                    branchId: branchId
                ...
                Vue.prototype.$message.success('解封成功');
                callback(result.success);
            });
        }
    }
