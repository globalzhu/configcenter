...
public class PropertyValueController {
    ...
    public EmptyResult addOrModifyPropertyValue(String appId, String profileId, String branchId, String key, String value, Scope scope, ValueType valueType) {
    ...
    @RequestMapping("/findAllPropertyValues")
    public FindPropertyValuesResult findAllPropertyValues(String profileId, String branchId, Scope minScope) {
        FindPropertyValuesOrder order = new FindPropertyValuesOrder();
        order.setAppId("ALL_QUERY");
        order.setProfileId(profileId);
        order.setBranchId(branchId);
        order.setMinScope(minScope);
    ...
    public ComparePropertyValuesWithReleaseResult comparePropertyValuesWithRelease(String appId, String profileId, String branchId, Long releaseVersion) {
        ...
        if(appId != null && "ALL_APP".equals(appId)){
            return result;
        }

        List<PropertyValueInfo> propertyValues = PropertyValues.findPropertyValues(appId, profileId, branchId, Scope.PRIVATE);
        Set<Property> left = propertyValues.stream().map(propertyValue -> new Property(propertyValue.getKey(), propertyValue.getValue(), propertyValue.getScope(),propertyValue.getValueType())).collect(Collectors.toSet());
